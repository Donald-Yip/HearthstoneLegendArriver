...
- **每个新回合开始只延时一次**（默认 `pre_action_delay_seconds`，可在 `config.py` / Web「延时设置」调整）给盒子更新推荐留时间；同一回合内多次出牌**不重复延时**，出牌更连贯。
...
- 关键调优值（OCR 置信度阈值、预处理缩放、各级延时）集中在下 `config.py`，便于统一调整。

## 🖥️ 实时日志浮窗（右上角）

自动对局开始时，屏幕**右上角**会出现一个**置顶半透明**小窗，实时滚动显示自动化日志，方便你在游戏里直观看到每一步：

- **自己回合开始**那一行（`轮到己方操作`）用**绿色**高亮，并会自动把炉石窗口**唤回前台**（等效鼠标点击，避免 OCR 被其他窗口挡住）；
- **📊 头部显示实时战绩**：已完成场数、胜场、负场、胜率；只在**真正打完一局**后计入，中止脚本**不清空**，可点「🔄 重置战绩」清零；
- `[推荐]` / `[执行]` → 白，`等待` → 灰；
- 日志**增量追加**：只有停在底部时才自动跟随，上滑后不会再被强制滚动；缓冲 **2000 行**，较早日志不会被隐藏，可滚动回看；
- 长日志**自动换行**，不再被水平截断；
- 底部有**延时进度条 + 文字说明**（如 `⏳ 回合 3 开始：开始 OCR（3/7s）`），延时行不再刷屏正文，正文聚焦推荐 / 执行 / 阶段等关键内容；
- **💾 保存日志**按钮：把当前对战日志写入项目根 `logs/` 子目录（`对战日志_<时间>.txt`），按钮与正文均有保存成功 / 失败反馈；
- **点击浮窗不会抢炉石前台**（`WS_EX_NOACTIVATE`），不会因为点了一下浮窗就中断自动对战；
- 按住**鼠标左键拖动**可把窗口挪到任意位置；
...
### 2026-08-26 · 引擎整合 + 设置统一 + 时序可配置

本批次将**上游（magicstrangelzb）引擎逻辑整体整合**进本项目，并把**全部设置收敛到单一 `config.py`**、延时**全部可在 Web 动态调整**：

- **上游引擎整合**：并入上游手动控制器 / 推荐适配器 / 推荐解析器 / 手牌动画延迟模块（`hand_entry_count` + `HandAnimationDelay`，1s/张），以及配套单元测试；推荐解析对地标 / 英雄技能统一了目标解析。
...
- **换牌时序采用上游实测值**（默认 `换牌前 18s / 识别后 1s / 换牌重试 5s`），且**全部可在 Web「⏱️ 延时设置」卡片修改**，写入 `ui_config.json` 的 `delays` 段即时生效（下次开局/下回合读取）。
- **换牌重试间隔独立**：不再复用"识别后缓冲"，避免上游把 `post_ocr` 设 0 后重试变成 0 秒忙等。默认 5s，可配置。
- **第一回合额外延时**：开局生效的全局卡（如「黑暗主教本尼迪塔斯」SW_448，日志 `TriggerKeyword=START_OF_GAME_KEYWORD`）每张追加延时（默认 3.5s/张，无固定基础），让盒子推荐有足够时间更新。
- **浮窗延时倒计时表**：换牌前等待 / 换牌重试 / 每个回合延时，浮窗底部进度条都会显示倒计时（`⏳ 第一回合延时（N 张开局生效卡）…`），不再是静默 `sleep`。
- **修复 `mulligan_slot_invalid` 死循环**：此前为兼容 `PowerTaskList` 前缀拓宽了日志解析，导致把友方手牌 `CONTROLLER` 误改（1→2），`my_hand_cards` 少算、换牌判无效无限重试。已回退为仅解析 `GameState` 权威行，换牌手牌计数恢复正确。

### 换牌阶段“成功却提示失败”修复
- 之前用“点击后的手牌”做基准判断换牌是否生效，Power.log 更新快时会误判为“点击未生效”并重复执行换牌流；现在**成功提交后不再重复执行**，只快速轮询到对局进入战斗回合。

- **启动只弹 Web，不抢炉石前台**：`web_ui.py` 启动时不再立刻把炉石切到前台（原前台守护改为启动 30 分钟后才做第一次校正）；只有真正点击「开始对战」或到点守护时才把炉石拉回前台。
- **浮窗交互优化**：点击浮窗不抢炉石前台（`WS_EX_NOACTIVATE`）、日志可回看且不再被强制滚动、自动换行、延时进度条 + 文字说明、`logs/` 子目录保存对战日志。
